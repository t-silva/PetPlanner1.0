...
public class intHistorico extends AppCompatActivity {
    ...
    SQLiteDatabase bd;
    PetDatabase db;
    List<Humor> loadHumor;
    List<Atividade> loadAtv;
    HumorDAO humorDAO ;
    AtvDAO atvDAO;
    float max;
...
//        });
    ...
    private void carrega(int idPet){
        try{
            ...
            humorDAO = db.humorDAO();
            atvDAO = db.atvDAO();
            loadHumor= humorDAO.getAllByIds(idPet);
            loadAtv = atvDAO.getAllByIds(idPet);


            setContentView(R.layout.historico_activity);
            Toast.makeText(this,loadHumor.get(0).getTimeStamp(),Toast.LENGTH_SHORT).show();
...
//            /*      Fim Iniciando Gráficos       */
    ...
    private ArrayList<BarEntry> getBarEntries(ArrayList<BarEntry> entries) {
        float a ;
        int b = 1;
        for (int i = 0; i< loadAtv.size();i++) {

                    a = loadAtv.get(i).getTempo();
    ...
    private ArrayList<Entry> getLineEntriesData(ArrayList<Entry> entries) {
        float y = 0f;
        int v = 1;
        for (int i = 0; i < loadHumor.size();i++){
            if (loadHumor.get(i).getStatus().equals(getString(R.string.humorExc))) {
                y = 4f;
            } else if (loadHumor.get(i).getStatus().equals(getString(R.string.humorHappy))) {
                y = 3f;
            } else if (loadHumor.get(i).getStatus().equals(getString(R.string.humorOk))) {
                y = 2f;
            } else if (loadHumor.get(i).getStatus().equals(getString(R.string.humorBad))) {
                y = 1f;
            }
